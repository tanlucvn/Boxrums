...
import { verifyAccessTokenIO } from '../utils/jwt.js';
    ...
    io.on('connection', (socket) => {
        socket.on('join', async (data) => {
            ...
            if (/notification:/.test(data.room)) {
                ...
                const notifications = await Notification.find({ to: new Types.ObjectId(jwtData.id), read: false })
            ...
            if (/pmCount:/.test(data.room)) {
                ...
                const dialogues = await Dialogue.find({
                    $or: [{
                        to: new Types.ObjectId(jwtData.id)
                    }, {
                        from: new Types.ObjectId(jwtData.id)
                    }]
        ...
        socket.on('readMessages', async (data) => {
            ...
            try {
                await Message.updateMany({ dialogueId: new Types.ObjectId(dialogueId), from }, { read: true })
